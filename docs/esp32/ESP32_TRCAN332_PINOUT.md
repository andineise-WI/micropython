# ESP32-PICO-MINI + TRCan332 Pinout Guide
...
```
               ...
               5──┤ SENSOR_VN       ★ -> GPIO4 ├──45 <- TRCan332 TXD
               6──┤ GPIO34          ★ -> GPIO5 ├──44 <- TRCan332 RXD
               7──┤ GPIO35               GPIO6 ├──43
               8──┤ GPIO32               GPIO7 ├──42
               9──┤ GPIO33               GPIO8 ├──41
              10──┤ GPIO25               GPIO9 ├──40
...
★ = TWAI Pins für TRCan332 Connection
```

## TRCan332 SO-8 Pinout

```
           TRCan332 (SO-8 Package)
           ┌─────────────────────┐
        1──┤ TXD             VCC ├──8  -> Nicht verwendet
           │                     │
...
```
ESP32-PICO-MINI-02-N8R2                    TRCan332 SO-8
┌─────────────────────┐                    ┌─────────────┐
│               GPIO4 ├────────────────────┤1 TXD        │
│               GPIO5 ├────────────────────┤4 RXD        │
│                3.3V ├────────────────────┤3 VCC        │
...
1. **Kurze Verbindungen:** GPIO4/5 zu TRCan332 so kurz wie möglich
2. **Ground Plane:** Durchgehende Massefläche unter den Bauteilen
3. **Entstörung:** 100nF Keramikkondensator nahe TRCan332 VCC
...
```
Node 1: ESP32 + TRCan332 ──── CAN Bus ──── Node 2: ESP32 + TRCan332
                 │                                        │
                120Ω                                    120Ω
...
```
        ┌── Node 1 (ESP32 + TRCan332)
        │
CAN Bus ├── Node 2 (ESP32 + TRCan332)  
        │
...
Diese Konfiguration bietet eine robuste und zuverlässige CAN-Bus-Implementierung für ESP32-basierte Projekte.
